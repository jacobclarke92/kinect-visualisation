...
function isLineIntersectingPolygon(pt1, pt2, polygon) {
	var testLine = [pt1,pt2];
	var intersections = 0;

	/*var midpoint = {x: (pt1.x+pt2.x)/2, y: (pt1.y+pt2.y)/2};
	if(!isPointInsidePolygon(midpoint,polygon)) {
		console.log('midpoint is out of polygon');
		return true;
	}
	*/
	...
	for (var i = 0; i < polygon.length; i++) {
	    ...
	    var result = intersectionLines(testLine, polygonLine);

	    if (pointsEqual(pt1, result) || pointsEqual(pt2, result)) {

	    }else{
	    	if (result.onLine1) intersections ++;
	    }
